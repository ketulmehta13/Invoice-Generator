import tkinter
from tkinter import ttk
from docxtpl import DocxTemplate
import datetime
from tkinter import messagebox
import sqlite3  
def connect_db():
    conn = sqlite3.connect('invoice_db.sqlite')
    cursor = conn.cursor()
    
    # Create table if it doesn't exist
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS invoices (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            invoice_id INTEGER,
            first_name TEXT,
            last_name TEXT,
            phone TEXT,
            quantity INTEGER,
            description TEXT,
            unit_price REAL,
            line_total REAL,
            subtotal REAL,
            salestax REAL,
            total REAL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    ''')
    conn.commit()
    return conn

def clear_item():
    qty_spinbox.delete(0, tkinter.END)
    qty_spinbox.insert(0, "1")
    desc_entry.delete(0, tkinter.END)
    price_spinbox.delete(0, tkinter.END)
    price_spinbox.insert(0, "0.0")

invoice_list = []

def add_item():
    qty = int(qty_spinbox.get())
    desc = desc_entry.get()
    price = float(price_spinbox.get())
    line_total = qty * price
    invoice_item = [qty, desc, price, line_total]
    tree.insert('', 0, values=invoice_item)
    clear_item()
    invoice_list.append(invoice_item)

def new_invoice():
     # Simply clear the form without generating an invoice
    first_name_entry.delete(0, tkinter.END)
    last_name_entry.delete(0, tkinter.END)
    phone_entry.delete(0, tkinter.END)
    clear_item()
    tree.delete(*tree.get_children())
    invoice_list.clear()

def generate_invoice():
    db = connect_db()
    cursor = db.cursor()

    first_name = first_name_entry.get()
    last_name = last_name_entry.get()
    phone = phone_entry.get()
    
    if not first_name and not last_name and not phone:
        messagebox.showerror("Error", "Please fill in all customer details")
        return
    
    if not invoice_list:
        messagebox.showerror("Error", "Invoice is empty. Please add items.")
        return

    subtotal = sum(item[3] for item in invoice_list)
    salestax = 0.1
    total = subtotal * (1 + salestax)

    # Insert the invoice first
    cursor.execute(
        """INSERT INTO invoices (first_name, last_name, phone, subtotal, salestax, total) 
        VALUES (?, ?, ?, ?, ?, ?)""",
        (first_name, last_name, phone, subtotal, salestax, total)
    )
    db.commit()
    invoice_id = cursor.lastrowid

    # Insert items linked to the invoice_id
    for item in invoice_list:
        cursor.execute(
            """INSERT INTO invoices 
            (invoice_id, first_name, last_name, phone, quantity, description, unit_price, line_total, subtotal, salestax, total) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (invoice_id, first_name, last_name, phone, item[0], item[1], item[2], item[3], subtotal, salestax, total)
        )

    db.commit()
    db.close()

    # Generate Word Document
    doc = DocxTemplate("invoice_template.docx")
    doc.render({
        "name": first_name + " " + last_name,
        "phone": phone,
        "invoice_list": invoice_list,
        "subtotal": subtotal,
        "salestax": str(salestax * 100) + "%",
        "total": total
    })
    
    doc_name = f"{first_name}_{last_name}_{datetime.datetime.now().strftime('%Y-%m-%d-%H%M%S')}.docx"
    doc.save(doc_name)

    messagebox.showinfo("Invoice Complete", "Invoice Saved Successfully!")
    new_invoice()

# GUI Setup (same as before)
window = tkinter.Tk()
window.title("Invoice Generator Form")

frame = tkinter.Frame(window)
frame.pack(padx=20, pady=10)

first_name_label = tkinter.Label(frame, text="First Name")
first_name_label.grid(row=0, column=0)
last_name_label = tkinter.Label(frame, text="Last Name")
last_name_label.grid(row=0, column=1)

first_name_entry = tkinter.Entry(frame)
last_name_entry = tkinter.Entry(frame)
first_name_entry.grid(row=1, column=0)
last_name_entry.grid(row=1, column=1)

phone_label = tkinter.Label(frame, text="Phone")
phone_label.grid(row=0, column=2)
phone_entry = tkinter.Entry(frame)
phone_entry.grid(row=1, column=2)

qty_label = tkinter.Label(frame, text="Qty")
qty_label.grid(row=2, column=0)
qty_spinbox = tkinter.Spinbox(frame, from_=1, to=100)
qty_spinbox.grid(row=3, column=0)

desc_label = tkinter.Label(frame, text="Description")
desc_label.grid(row=2, column=1)
desc_entry = tkinter.Entry(frame)
desc_entry.grid(row=3, column=1)

price_label = tkinter.Label(frame, text="Unit Price")
price_label.grid(row=2, column=2)
price_spinbox = tkinter.Spinbox(frame, from_=0.0, to=500, increment=0.5)
price_spinbox.grid(row=3, column=2)

add_item_button = tkinter.Button(frame, text="Add item", command=add_item)
add_item_button.grid(row=4, column=2, pady=5)

columns = ('qty', 'desc', 'price', 'total')
tree = ttk.Treeview(frame, columns=columns, show="headings")
tree.heading('qty', text='Qty')
tree.heading('desc', text='Description')
tree.heading('price', text='Unit Price')
tree.heading('total', text="Total")

tree.grid(row=5, column=0, columnspan=3, padx=20, pady=10)

save_invoice_button = tkinter.Button(frame, text="Generate Invoice", command=generate_invoice)
save_invoice_button.grid(row=6, column=0, columnspan=3, sticky="news", padx=20, pady=5)
new_invoice_button = tkinter.Button(frame, text="New Invoice", command=new_invoice)
new_invoice_button.grid(row=7, column=0, columnspan=3, sticky="news", padx=20, pady=5)

window.mainloop()
